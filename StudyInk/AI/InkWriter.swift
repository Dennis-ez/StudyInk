...
/// outline (from a light hand-style font) is traced as real strokes — erasable,
/// lassoable, undoable, exported like the user's own ink. A LIGHT face plus a
/// stem-matched stroke fills the letters solid (a thin trace looks hollow; a
/// heavy bold face looks fat). Used by the ambient tutor / Answer in Ink.
enum InkWriter {
    ...
    /// `strokeWidth` traces the glyph outline; a stem-width value fills it solid.
